...
<div align="center">
  <img src="assets/spycut-icon.svg" width="112" alt="SpyCut 图标" />
  <h1>SpyCut</h1>
  <p><strong>为长录屏而生的精确删除式剪辑器</strong></p>
  <p>删掉不该出现的，其余内容严格保持原位。</p>
</div>
...
[下载预览版](https://github.com/songpy97/spycut/releases) · [报告问题](https://github.com/songpy97/spycut/issues) · [参与贡献](CONTRIBUTING.zh-CN.md)
...
![SpyCut 主编辑器：音频波形、删除区间和原始时间轴](assets/screenshots/editor-overview.jpg)

## 为什么是 SpyCut

传统剪辑软件要求你管理大量“保留片段”；SpyCut 只让你标记少量“删除区间”。保留内容始终由删除区间的补集确定，顺序不会被误改。这种约束特别适合长录屏的公开前清理：工作流更简单，也更容易检查。

- **网课与教程**：删除口误、等待、课间休息和不应公开的答疑。
- **Webinar 与企业培训**：移除会前准备、内部讨论和敏感片段。
- **产品演示与编程录屏**：清理失败尝试、加载等待和重复讲解。
- **会议与社区活动录像**：在发布前逐处确认隐私敏感的连接点。
- **专业剪辑前置清理**：先快速生成干净的长片，再进入 Premiere、Final Cut 或 DaVinci Resolve 精修。

SpyCut 全程在本机处理媒体，不需要上传源视频；源文件只读，导出在自动验收通过后才提交最终文件。

> 当前预览版界面为简体中文。英文 UI 本地化已列入后续计划；英文 README、构建说明和协作入口已经可用。

## 产品界面

| 逐处复核删除连接点 | 受监督的精确导出 |
| --- | --- |
| ![逐处复核删除连接点](assets/screenshots/join-review.jpg) | ![显示编码器与进度的精确导出](assets/screenshots/export-progress.jpg) |
...
5. 长录屏可用 `Alt/Option + 滚轮` 或 `Ctrl + 滚轮` 以鼠标位置为中心连续缩放时间轴；触控板横向滑动或 `Shift + 滚轮` 可平移，底部细导航条用于跨小时快速定位，“适配全片”可恢复全局视图。
...
脚本会提示选择 major（大版本）、minor（中版本）或 patch（小版本），同步 `package.json`、Tauri、Cargo 和 lockfile 版本，运行 `pnpm check`，并在最终确认后把当前工作区的全部改动提交为 release commit。脚本禁用 Git 分页器，所有检查输出结束后会直接显示最终确认，不会停在 `END`。发布检查会跳过会再次启动 commit/tag/push 的嵌套发布模拟，其余前端、类型与 Rust 门禁照常执行；该发布模拟在日常测试和 CI 中仍会运行。没有全局 `pnpm` 时，脚本会优先通过 Corepack，否则通过 `npm` 临时使用仓库固定的 `pnpm@11.16.0`（首次使用 npm 回退时需要联网下载）。随后它创建注解标签，并原子推送 `main` 与 `v*` 标签；如果提交或本地标签已创建但尚未推送，下次运行会检查并提交待处理修复、把未发布标签更新到当前 `main`，再继续原子推送，而不会误增下一个版本。检查失败或最终确认前取消时会恢复版本文件。运行前务必查看脚本显示的 `git status`，确认当前所有改动都应进入本次发布。
