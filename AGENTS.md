...
VFR 视频和音频必须先按解码器原始 `t` 筛选。视频筛选后用 `settb=AVTB` 保持秒值并显式重标 timebase，再以 `T/STARTT` 扣除已结束删除区间的累计墙钟时长并通过 `fps` 生成 CFR；音频筛选后用 `asettb=AVTB` 重标，并依据保留样本数连续重排。自动验收必须以删除区间补集的墙钟时长为准，不能放宽容差来接受按帧数或错误 timebase 压缩、拉长的成片。
