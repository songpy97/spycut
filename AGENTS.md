...
- VFR 输入导出时必须先在解码器提供的原始墙钟时间轴筛选；筛选后显式重标 timebase、压缩视频 PTS，再统一转换为 CFR。禁止用保留帧数和全片平均帧率推导成片时长，也禁止在筛选前用裸 `PTS-STARTPTS` 改写轨道时间基语义。
...
VFR 视频和音频必须先按解码器原始 `t` 筛选。视频筛选后用 `settb=AVTB` 保持秒值并显式重标 timebase，再以 `T/STARTT` 扣除当前源时间与所有删除区间的累计重叠墙钟时长并通过 `fps` 生成 CFR；只扣除已经结束的完整删除区间会在视频 EOF 落入尾删区间时错误补帧。音频必须用 `asegment` 在原始时间轴上按删除边界顺序分段，丢弃删除段，将保留段从零重排并串接，最后依据保留样本数生成连续 PTS；每个 `asegment` 和 `concat` 节点的扇入、扇出必须保持固定小上限，禁止把全部边界或保留段集中到单个宽节点。不得依赖 FFmpeg 8.0.1 中失效的 `aselect`，也不得为每个保留区间复制整条长音轨。自动验收必须以删除区间补集的墙钟时长为准，不能放宽容差来接受按帧数或错误 timebase 压缩、拉长的成片。
...
交互式发布脚本会把运行时工作区的全部改动纳入 release commit；确认发布前必须复核其 `git status` 和 diff 摘要。脚本内必须禁用 Git 分页器，检查输出不得停在 `less` 的 `END` 而阻断最终确认。发布脚本内部的 `pnpm check` 只跳过会再次执行 commit/tag/push 的嵌套发布模拟，该模拟在普通测试和 CI 中仍必须运行。检查失败或最终确认前取消时脚本应恢复四个版本文件，开始提交后不得自动重写已发布的 Git 历史；release commit 或本地标签已经创建但远端尚无该标签时，续发流程可在明确确认后检查并提交剩余改动，将仅存在于本地的标签更新到当前 `main` 后原子推送。原子 push 失败时保留本地 release commit 和标签供后续续推，禁止 force push 或复用已发布标签。
