...
- VFR 输入导出时必须按保留内容的原始墙钟时间压缩视频 PTS，再统一转换为 CFR；禁止用保留帧数和全片平均帧率推导成片时长。
...
VFR 视频筛选前先把输入 PTS 归零；筛选后从视频 PTS 中扣除已结束删除区间的累计墙钟时长，再通过 `fps` 过滤器生成 CFR。音频继续依据保留样本数连续重排。自动验收必须以删除区间补集的墙钟时长为准，不能放宽容差来接受按帧数压缩或拉长的成片。
