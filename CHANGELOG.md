...
## 1.0.0 首个公开 1.0 预览版 - 2026-08-14

- 将版本统一升级到 `1.0.0`，继续以未签名、未公证的预发布形式提供 macOS Apple Silicon 与 Windows x64 原生安装包。
- 修复新版 FFmpeg 已移除 `-filter_complex_script` 别名导致精确导出失败的问题，改用受支持的文件式 `-/filter_complex` 参数并增加命令回归测试。
- 完善长录屏键盘和滚轮操作：空格键在按钮聚焦时仍可控制播放，`Ctrl + 滚轮` 可缩放时间轴，并保留逐帧、1 秒、5 秒与 30 秒导航。
- 交付英文默认 README、简体中文切换、真实合成演示截图、国际化贡献与安全指南，以及隐私友好的 Issue / Pull Request 模板。
- 保持删除式不可重排时间轴、源文件只读、连接点逐处复核、同盘临时导出、自动媒体验收和失败不污染目标文件等核心约束。
