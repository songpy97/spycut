...
The script prompts for a major, minor, or patch increment, synchronizes the package, Tauri, Cargo, and lockfile versions, runs `pnpm check`, then asks for final confirmation before including every current worktree change in a release commit. It disables the Git pager so checks lead directly to the final prompt instead of stopping at `END`. During an actual release check it skips only the nested integration case that would launch another commit/tag/push simulation; that case still runs in ordinary tests and CI. If global `pnpm` is unavailable, the script uses Corepack or falls back to downloading the repository-pinned `pnpm@11.16.0` temporarily through `npm` (the first npm fallback requires network access). It creates an annotated `v*` tag and atomically pushes `main` plus the tag. If a release commit or local tag was created but not pushed, the next run checks and commits any pending fix, moves only that unpublished tag to the current `main`, and continues the atomic push without incrementing the version again. That tag triggers native ARM64 macOS, Intel macOS, and Windows packaging; all three must pass before the workflow creates a prerelease. A manual `workflow_dispatch` uploads artifacts but does not create a GitHub Release.
